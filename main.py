...
PERSONAS = {
    ...
    "rustam": {
        ...
        "system": (
            ...
            "Работаешь с API Bitget. Говоришь чётко, по-военному. "
            "Отвечаешь на русском. Используешь HTML теги <b> и <i>. Максимум 200 слов."
        ),
    },
    "adrian": {
        ...
        "system": (
            ...
            "Следишь за макроэкономикой и новостями. Думаешь глобально. "
            "Отвечаешь на русском. Используешь HTML теги <b> и <i>. Максимум 200 слов."
        ),
    },
}
...
async def fetch_market_data() -> dict:
    ...
    try:
        ...
        return {
            "price": ticker["last"], "change": ticker.get("percentage",0) or 0,
            "volume": ticker.get("quoteVolume",0) or 0,
            "high24": high24, "low24": low24, "ma7": ma7, "ma24": ma24, "rsi": rsi,
            "fib": {
                "0.236": round(low24+0.236*fib_r,2), "0.382": round(low24+0.382*fib_r,2),
                "0.500": round(low24+0.500*fib_r,2), "0.618": round(low24+0.618*fib_r,2),
            },
            "bid_wall": bid_wall, "ask_wall": ask_wall,
            "avg_volume": sum(volumes)/len(volumes), "closes": closes,
        }
    finally:
...
async def run_consilium(chat_id: int):
    ...
    for agent_key, task in CONSILIUM_PLAN:
        ...
        await bots[agent_key].send_message(chat_id, f"<b>{display}</b>\n\n{answer}", parse_mode=ParseMode.HTML)
        await asyncio.sleep(random.uniform(3, 5))

MARKET_KEYWORDS = ["курс","цена","рынок","btc","анализ","тренд","риск","стоп","вход","сделка","покупать","продавать","фибоначчи","rsi","объём"]
...
def register_handlers(key: str, dp: Dispatcher):
    ...
    async def cmd_start(message: types.Message):
        ...
        await message.reply(f"👋 <b>{display}</b> в сети!\nНазывай моё имя — отвечу.\n<b>/work</b> — консилиум\n<b>/status</b> — Bitget", parse_mode=ParseMode.HTML)
    ...
    async def cmd_work(message: types.Message):
        ...
        if not is_owner(message):
            ...
            return
        await message.reply("⚡️ <b>Консилиум запущен!</b>", parse_mode=ParseMode.HTML)
